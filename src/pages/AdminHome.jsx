...
import PenjualanDetail from "../components/PenjualanDetail";
import { myAxios } from "../utils/axios";

export default function AdminHome() {
  const [delivers, setDelivers] = useFetchGet(
    "/api/penjualan"
  );
  ...
  return (
    <>
      <div className='w-full flex flex-col items-center gap-y-6'>
        <h1 className='font-bold text-5xl mt-10 font-geologica'>Order List</h1>
        {msg && (
          ...
          <div className='overflow-x-auto grow'>
            <table className='w-full text-center text-sm '>
              <thead className='border-b bg-neutral-800 font-medium text-white dark:border-neutral-500 dark:bg-neutral-900 min-h-fit'>
                <tr>
                  <th scope='col' className='whitespace-nowrap px-6 py-4 w-1/6'>
                    Number
                  </th>
                  <th scope='col' className='whitespace-nowrap px-6 py-4 w-2/6'>
                    Username
                  </th>
                  <th scope='col' className='whitespace-nowrap px-6 py-4 w-1/6'>
                    Date
                  ...
                  return (
                    <tr key={i} className='border-b dark:border-neutral-500'>
                      ...
                      <td className='whitespace-nowrap  px-6 py-4 flex gap-x-4 justify-evenly'>
                        <button
                          onClick={() => {
                            setPenjualanDetail(!penjualanDetail);
                            setItems(d.item_terjual);
                          }}
                        ...
                        <button
                          onClick={() => {
                            setStatusSetting(!statusSetting);
                            setId_penjualan(d.id_penjualan);
                          }}
