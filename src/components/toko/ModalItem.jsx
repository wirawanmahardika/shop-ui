...
import { addToCart, editCart } from "../../utils/redux/slice/CartItem";
        ...
        <>
            {data && (
                <>
                    <div
                        onClick={setToggle}
                        className="fixed z-[100] top-0 right-0 left-0 bottom-0 blur-md backdrop-blur-[2px] block"
                    ></div>
                    <div className="flex  overflow-y-auto w-4/5 lg:w-3/5 z-[101] rounded-md fixed left-1/2 top-1/2 py-10 p-3 -translate-x-1/2 -translate-y-1/2 bg-black text-white max-h-full flex-col gap-y-6">
                        ...
                        <div className="sm:grid sm:grid-cols-3 sm:grid-rows-1 sm:items-center flex flex-col gap-y-4">
                            ...
                            <button
                                ...
                                onClick={() => {
                                    if (
                                        cartItems.find((d) => d.id === data.id)
                                    ) {
                                        dispatch(
                                            editCart({ id: data.id, qty })
                                        );
                                        toast.success(
                                            "Item telah ditambahkan ke cart",
                                            {
                                                position: "top-center",
                                                autoClose: 5000,
